...
//        }
    ...
    fun updateData(@Nullable data: List<NewsTable>) {
        NewsLogUtil.d("headlines "+ isHeadline +" newsDetail adapter "+data.size)
        this.newsData = data
         ...
         fun bind() {
             if (isHeadline){
                 ...
                 binding!!.shareNews.setOnClickListener({
                     mClickHandler.onShareClick(newsData!![adapterPosition].url)
                 })
             }
         }
...
//            }
//            mClickHandler.onItemClick(newsData!![newPostion].url)
            val direction =NewsListFragmentDirections.actionNewsDetails(newsData!![newPostion].id)
            v!!.findNavController().navigate(direction)
            NewsFirebaseLogging.logEvent("view_news_screen",mContext)
       }
